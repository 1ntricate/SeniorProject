...
def Knn(input_vector, known_data, k, threshold):
    ...
    for i in range(k):
        ...
        if d_d <= threshold:
            #print(f"{i+1} distance: {d_d} Label: {type}")
...
# Moderate Emphasis on Color
elif slider_value == 25:
    weights =  [0.225, 0.225, 0.225, 0.065, 0.065, 0.065, 0.065, 0.065]
#Strong Emphasis on Color
elif slider_value == 0:
    weights = [0.275, 0.275, 0.275, 0.035, 0.035, 0.035, 0.035, 0.035]
# Moderate Emphasis on Edges
elif slider_value == 75:
    weights = [0.111, 0.111, 0.111, 0.225, 0.111, 0.111, 0.111, 0.111]
# Strong Emphasis on Edges
elif slider_value ==100:
    weights =  [0.104, 0.104, 0.104, 0.275, 0.104, 0.104, 0.104, 0.104]
...
min_values = np.min(combined_values, axis=0)
max_values = np.max(combined_values, axis=0)

min_max_normalized_feature_vectors = [
    [(feature_value - min_val) / (max_val - min_val) * weight
     for feature_value, min_val, max_val, weight in zip(feature_vector, 
                                min_values, max_values, weights)]
    for feature_vector in feature_vectors_values
]

min_max_normalized_known_data = [
    [(data_value - min_val) / (max_val - min_val) * weight
     for data_value, min_val, max_val, weight in zip(data, min_values, max_values, weights)]
    for data in known_data_values
]

# Re-add labels to the normalized lists
normalized_input_features = [(normalized_feature_vector, feature_vector[8]) for normalized_feature_vector, feature_vector in zip(min_max_normalized_feature_vectors, feature_vectors)]
normalized_known_features = [(normalized_data, data[1], data[2]) for normalized_data, data in zip(min_max_normalized_known_data, known_data)]
type_counts = {}
# Knn classification
for i, feature_vector in enumerate(normalized_input_features):
    ...
    patch_2 = patches[i]
    patch_num = i + 1
    # Update the type_counts dictionary
    if type in type_counts:
        type_counts[type] += 1
    else:
        type_counts[type] = 1
